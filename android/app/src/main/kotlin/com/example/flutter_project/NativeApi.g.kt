...
data class Cordinates (
 ...
 {
  companion object {
    fun fromList(pigeonVar_list: List<Any?>): Cordinates {
      val latitude = pigeonVar_list[0] as Double?
      val longitude = pigeonVar_list[1] as Double?
      return Cordinates(latitude, longitude)
    }
  }
  fun toList(): List<Any?> {
    return listOf(
...
interface NativeApi {
  ...
  companion object {
    ...
    fun setUp(binaryMessenger: BinaryMessenger, api: NativeApi?, messageChannelSuffix: String = "") {
      ...
      run {
        val channel = BasicMessageChannel<Any?>(binaryMessenger, "dev.flutter.pigeon.flutter_project.NativeApi.sendMessage$separatedMessageChannelSuffix", codec)
        if (api != null) {
          channel.setMessageHandler { message, reply ->
            val args = message as List<Any?>
            val msgArg = args[0] as Message
            val wrapped: List<Any?> = try {
              listOf(api.sendMessage(msgArg))
            } catch (exception: Throwable) {
              NativeApiPigeonUtils.wrapError(exception)
            }
            reply.reply(wrapped)
          }
        } else {
      ...
      run {
        val channel = BasicMessageChannel<Any?>(binaryMessenger, "dev.flutter.pigeon.flutter_project.NativeApi.getCoordinates$separatedMessageChannelSuffix", codec)
        if (api != null) {
          channel.setMessageHandler { _, reply ->
            val wrapped: List<Any?> = try {
              listOf(api.getCoordinates())
            } catch (exception: Throwable) {
              NativeApiPigeonUtils.wrapError(exception)
            }
            reply.reply(wrapped)
